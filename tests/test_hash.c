...
void print_digest(const char* label, const uchar_t* d, int len)
{
    printf("%s: ", label);
...
int main()
{
    ...
    uchar_t *hash = malloc(sizeof(uchar_t)*32) ; 
    
    sha256_hash(input , length , hash) ;
    printf("final result  : \n" );
    PRINT_ARRAY_NOSPC(hash , 32 , "%02x") ; 
    
    md4_hash(input , length , hash) ;
    printf("final result  : \n" );
    PRINT_ARRAY_NOSPC(hash , 16 , "%02x") ; 
    
    
    md5_hash(input , length , hash) ;
    printf("final result  : \n" );
    PRINT_ARRAY_NOSPC(hash , 16 , "%02x") ; 
...
*/    
    ...
    // // Test 1: empty string
    // uchar_t d[64];
    // md5_hash((uchar_t*)"", 0, d);
    // print_digest("MD5(\"\")", d, 16);

    // // Test 2: 1 byte
    // md5_hash((uchar_t*)"A", 1, d);
    // print_digest("MD5(\"A\")", d, 16);

    // // Test 3: 1 KB
    // uchar_t* kb = malloc(1024);
    // memset(kb, 'B', 1024);
    // md5_hash(kb, 1024, d);
    ...
    // for (int i = 0; i < 10; i++) {
    //     sha256_hash((uchar_t*)vectors[i], strlen(vectors[i]), d);
    ...
    // t0 = clock();
    // md5_hash((uchar_t*)msg, strlen(msg), d);
    // t1 = clock();
    // print_digest("MD5   ", d, 16);
    // printf("  Time: %.6f ms\n", (t1 - t0) * 1000.0 / CLOCKS_PER_SEC);

    // t0 = clock();
    // sha256_hash((uchar_t*)msg, strlen(msg), d);
    // t1 = clock();
    // print_digest("SHA256", d, 32);
    // printf("  Time: %.6f ms\n", (t1 - t0) * 1000.0 / CLOCKS_PER_SEC);

    // t0 = clock();
    // sha512_hash((uchar_t*)msg, strlen(msg), d);
    // t1 = clock();
    // print_digest("SHA512", d, 64);
    // printf("  Time: %.6f ms\n", (t1 - t0) * 1000.0 / CLOCKS_PER_SEC);

    // // Avalanche for all three
    // av = avalanche_test((uchar_t*)msg, strlen(msg), md5_hash, 16);
    // printf("\nMD5    avalanche: %.1f%%\n", av);
    // av = avalanche_test((uchar_t*)msg, strlen(msg), sha256_hash, 32);
    // printf("SHA256 avalanche: %.1f%%\n", av);
    // av = avalanche_test((uchar_t*)msg, strlen(msg), sha512_hash, 64);
    ...
    // {
    //     uchar_t key[20]; memset(key, 0x0b, 20);
    //     const uchar_t* data = (const uchar_t*)"Hi There";
    //     uchar_t mac[32];
    //     const uchar_t expected[32] = {
    ...
    // {
    //     const uchar_t* key = (const uchar_t*)"Jefe";
    //     const uchar_t* data = (const uchar_t*)"what do ya want for nothing?";
    //     uchar_t mac[32];
    //     const uchar_t expected[32] = {
    ...
    // {
    //     uchar_t key[131]; memset(key, 0xaa, 131);
    //     const uchar_t* data = (const uchar_t*)
    ...
    // {
    //     uchar_t ikm[22]; memset(ikm, 0x0b, 22);
    //     const uchar_t salt[13] = {
    //         0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0a,0x0b,0x0c
    //     };
    //     const uchar_t info[10] = {
    //         0xf0,0xf1,0xf2,0xf3,0xf4,0xf5,0xf6,0xf7,0xf8,0xf9
    //     };
    //     const uchar_t expected_prk[32] = {
    //         0x07,0x77,0x09,0x36,0x2c,0x2e,0x32,0xdf,0x0d,0xdc,0x3f,0x0d,0xc4,0x7b,0xba,0x63,
    //         0x90,0xb6,0xc7,0x3b,0xb5,0x0f,0x9c,0x31,0x22,0xec,0x84,0x4a,0xd7,0xc2,0xb3,0xe5
    //     };
    //     const uchar_t expected_okm[42] = {
    //         0x3c,0xb2,0x5f,0x25,0xfa,0xac,0xd5,0x7a,0x90,0x43,0x4f,0x64,0xd0,0x36,0x2f,0x2a,
    //         0x2d,0x2d,0x0a,0x90,0xcf,0x1a,0x5a,0x4c,0x5d,0xb0,0x2d,0x56,0xec,0xc4,0xc5,0xbf,
    //         0x34,0x00,0x72,0x08,0xd5,0xb8,0x87,0x18,0x58,0x65
    //     };

    //     uchar_t prk[32], okm[42];
