...
void substittution_set_key(void* key_struct, const CString key_str) {
    ...
    if (substitution_key->alphabet_size == 26)
    {
        ...
        if ((int) strlen((const char *) key_str.string) != substitution_key->alphabet_size   )
        {
            printf("ERROR: key too short to be used in substitution encryption\n");
            return ; 
        }
        
        if (!areElementsUniqueSorted((const uchar_t*) key_str.string , substitution_key->alphabet_size)   )
        {
        ...
        // copy it
        strcpy((char *) substitution_key->map , key_str.string) ; 
