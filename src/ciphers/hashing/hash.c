...
// Construction: H((K' ^ opad) || H((K' ^ ipad) || M))
// where K' is the key padded to the block size (hashed first if oversized).
                 ...
                 uint8_t mac[32])
{
    ...
    if (key_len > SHA256_BLOCK_SIZE) {
        // Oversized key: collapse with the underlying hash, remainder stays zero.
...
// HMAC-SHA512: same construction with a 128-byte block.
                 ...
                 uint8_t mac[64])
{
    ...
    memset(k_block, 0, SHA512_BLOCK_SIZE);
    if (key_len > SHA512_BLOCK_SIZE) {
        sha512_hash(key, key_len, k_block);
    } else {
        memcpy(k_block, key, key_len);
    }
