...
static uint32_t blowfish_ffunc(uint32_t input ,BlowfishKey blowfish_key   ) {
    uchar_t a = (input >> 24) & 0xff ; 
    uchar_t b = (input >> 16) & 0xff ; 
    uchar_t c = (input >> 8) & 0xff ; 
    uchar_t d = input & 0xff ; 

    uint32_t output = blowfish_key.s1[a] + blowfish_key.s2[b] ; 

    output ^= blowfish_key.s3[c] ; 

    output += blowfish_key.s4[d] ; 
...
void blowfish_encrypt_block(const uchar_t* input, uchar_t* output  , const void* key) {
    ...
    // swap left and right  
    // lets pretend im optimizing for the code to be run on small ship and use XOR instead of temp variable *-* (it just looks cooler)
    left = right ^ left ;
    // left now has ` right ^ left `  
    
    right = left ^ right ;
    // right now has ` right ^ left ^ right = left  `
    
    
    left = right ^ left ;
    // left now has ` left ^ right ^ left = right  `
...
void blowfish_set_key(void* key_struct, const uchar_t* key_str , size_t key_len) {
    ...
    */
    ...
    // TODO: maybe i should get back here and calculate the (18 + 1024) * 32 digit of pi manually :( ... 
    uint32_t sbox0[] = {
    ...
    memcpy(blowfish_key->p , parray , sizeof(uint32_t)*18) ; 
    
    memcpy(blowfish_key->s1 , sbox0 , sizeof(uint32_t)*256) ; 
    memcpy(blowfish_key->s2 , sbox1 , sizeof(uint32_t)*256) ; 
    memcpy(blowfish_key->s3 , sbox2 , sizeof(uint32_t)*256) ; 
    memcpy(blowfish_key->s4 , sbox3 , sizeof(uint32_t)*256) ; 
    ...
    // now the boring routine ...
        ...
        for (size_t i = 0; i < 18; i++)
        {
            ...
            blowfish_key->p[i] ^= bytes_to_uint32_t(&blowfish_key->key[(i*4) % blowfish_key->length]) ; ; 
        }
    

    uchar_t buffer[8] = {0 , 0 , 0 , 0 , 0 , 0 , 0 , 0}  ; 
    ...
    {   
        ...
        uint32_t_to_bytes(blowfish_key->p[i] , buffer) ; 
        uint32_t_to_bytes(blowfish_key->p[i+1] , buffer + 4) ; 
    ...
    {   
        ...
        uint32_t_to_bytes(blowfish_key->s1[i] , buffer) ; 
        uint32_t_to_bytes(blowfish_key->s1[i+1] , buffer + 4) ; 
    ...
    {   
        ...
        uint32_t_to_bytes(blowfish_key->s2[i] , buffer) ; 
        uint32_t_to_bytes(blowfish_key->s2[i+1] , buffer + 4) ; 
    ...
    {   
        ...
        uint32_t_to_bytes(blowfish_key->s3[i] , buffer) ; 
        uint32_t_to_bytes(blowfish_key->s3[i+1] , buffer + 4) ; 
    ...
    {   
        ...
        uint32_t_to_bytes(blowfish_key->s4[i] , buffer) ; 
        uint32_t_to_bytes(blowfish_key->s4[i+1] , buffer + 4) ; 
...
// man this is not healthy at all 🐡🐡🐡🐡🐡🐡🐡🐡🐡🐡🐡🐡🐡🐡🐡...
